...
<CardGrid>
  <LinkCard title="Case Study: Kubernetes from a NixOS Fleet" href="/tutorials/case-study-kubernetes/" description="One source of truth for hosts, BGP and the cluster manifests that depend on them." />
  <LinkCard title="Case Study: Access Control & Environments" href="/guides/acl-environments/" description="Derive who logs in where, and with which groups, from one group graph." />
  <LinkCard title="Quirk Recipes" href="/guides/quirk-recipes/" description="Language aspects that configure every editor, per-aspect persistence, shared overlays." />
  <LinkCard title="Aspect Settings" href="/guides/aspect-settings/" description="Typed per-host tunables declared next to the aspect that reads them." />
  <LinkCard title="Fleet Demo" href="/tutorials/fleet-demo/" description="Environments, many hosts and cross-host data in a runnable template." />
  <LinkCard title="MicroVM" href="/tutorials/microvm/" description="Guests as den entities: extending the pipeline with a new kind." />
</CardGrid>
...
3. **Resolution pipeline** -- `den.policies` fan out from entity kinds (`{host}` → `{host, user}`, `{home}`) and bind behavior. Batteries add their own classes (`homeManager`, `hjem`, `wsl`, …) and, where needed, derived entity kinds such as `wsl-host`.
