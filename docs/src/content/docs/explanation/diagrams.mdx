...
Capture the fleet once in den, then cut out the scope you want to draw:

```nix
diag = inputs.den-diagram.lib;
fleetCapture = den.lib.capture.captureFleet { };

# one host's subtree, including its users
g = diag.projectScope { inherit fleetCapture; kind = "host"; name = "igloo"; };
...
```nix
diag.toMermaid (diag.graph.simplified g)
```

Or slice to a single class:

```nix
diag.toMermaid (diag.graph.classSlice "nixos" g)
```

## Building graphs

- **`projectScope { fleetCapture; kind; name; direction?; }`** -- the graph
  as seen from one scope of a fleet capture: the scope and all its
  descendants. `kind` is any entity kind (`host`, `user`, `environment`, …).
  This is the usual entry point, since one `captureFleet` serves every view.
- **`context { entries; ctxTrace?; pathsByClass?; name; direction?; }`** --
  builds a graph from a standalone capture of a single root:
...
The [diagram case study](/tutorials/case-study-diagrams/) wires this into a
flake and shows the generated output.
...
These are illustrative, not the full set. See the [diagram reference](/reference/diag/) for the complete catalogue of predicate, closure, reshape, fold, and composite filters.
...
Den ships many more (C4, sankey, treemap, mindmap, state, JSON, and fleet-wide variants). See the [diagram reference](/reference/diag/) for the full list grouped by family.
...
entries = diag.export.entityEntries { inherit pkgs rc diag; } {
  entity = diag.projectScope { inherit fleetCapture; kind = "host"; inherit (host) name; };
  name = host.name;
  dir = "hosts/${host.name}";
...
- [diagram reference](/reference/diag) -- complete list of wrappers, filters, renderers, themes, and export helpers
