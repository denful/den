...
```mermaid
flowchart TD
  S["aspect at scope S destructures an entity kind, e.g. user"] --> Q1{"kind already in S's context?"}
  Q1 -- yes --> B["bind once, emit at S"]
  Q1 -- no --> Q2{"kind is a schema descendant of S?"}
  Q2 -- yes --> FO["fan out once per descendant, each emits at S"]
  Q2 -- no --> IN["inert: contributes nothing, no warning"]
```
...
den.aspects.laptop = { host, ... }: {
  nixos.networking.hostName = host.name;
  homeManager.home.sessionVariables.HOST = host.hostName;
};
```
...
den.aspects.desktop = {
  nixos = { user, ... }: {
    users.users.${user.userName}.extraGroups = [ "video" "input" ];
  };
};
```
