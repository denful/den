...
```nix
host.hasAspect den.aspects.zfs-root        # bool — primary class
host.hasAspect.forClass "nixos" den.aspects.zfs-root
host.hasAspect.forAnyClass den.aspects.zfs-root   # union across the entity's classes
```
...
of **all resolved aspect nodes** (every depth) for its primary class. The
entity root itself and excluded/tombstoned aspects are not listed; anonymous
aspects are. Each node keeps its `.name`, `.meta`, and `.includes` (its
...
The entity's `hasAspect` record also carries `aspectsForClass` and
`allAspects` (union across classes) when you need them via the lib-level
constructors below.
...
den.aspects.secrets-bundle = {
  ...
  # Prefer agenix: drop sops-nix from this subtree whenever both are present.
