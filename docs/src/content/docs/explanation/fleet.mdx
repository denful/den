...
When a Den flake declares multiple hosts, they all resolve in a single
pipeline run. This is what makes cross-host data flow possible — hosts
are sibling scopes in a shared scope tree, and `pipe.collect` can reach
across them.
