...
# Every entity's `.aspect` (default: `den.aspects.<name>`) is its entry point;
# the pipeline resolves it without any include.

# host -> users: fan-out policy (modules/policies/core.nix, simplified)
...
# conditional: home-manager only for users listing the class
# (nix/lib/home-env.nix, simplified)
den.policies.host-to-hm-users = { host, ... }:
  lib.optionals host.home-manager.enable (
    map (user: policy.resolve { inherit user; })
      (lib.filter (u: lib.elem "homeManager" u.classes) (lib.attrValues host.users)));

# host -> wsl-host: same data shape, different entity kind
den.policies.host-to-wsl-host = { host, ... }:
  lib.optional (host.class == "nixos" && host.wsl.enable)
...
[home-env](https://github.com/denful/den/blob/main/nix/lib/home-env.nix) is how Den implements homeManager/hjem/maid support
