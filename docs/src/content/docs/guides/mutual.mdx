...
:::note
Content pushed from the host side (host policies, `provides.<user>` on the host, `host-aspects`) does not reach a standalone `user@host` home. Put host-specific config on the user side, as below. See [Standalone Home Manager](/guides/standalone-home-manager/).
:::
