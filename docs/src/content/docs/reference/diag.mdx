...
Core IR builder. Transforms a list of structured trace entries into a graph record containing `nodes`, `edges`, `stages`, `stageEdges`, `rootId`, `rootName`, and `direction`. This is the lowest-level constructor -- `projectScope` and `context` call it internally.

### `projectScope { fleetCapture, kind, name, direction? }`

The graph as seen from one scope of a `den.lib.capture.captureFleet` result: the scope and all its descendants. `kind` is any entity kind (`host`, `user`, `environment`, …). The usual entry point for per-entity views.

### `context { entries, ctxTrace?, pathsByClass?, name, direction? }`

Builds a graph from a standalone capture (`captureWithPathsWith` and friends) of a single root.
...
|---|---|---|
| `userDeclaredOnly` | `graph -> graph` | User-authored aspects: those with class content, plus aspects that include them (policies excluded) |
...
Fleet-level capture: runs the **entire** flake scope tree (flake → fleet →
host → user) from the flake root, not a single entity. Returns the trace
entries **plus** post-pipeline scope state for pipe-flow analysis:

```nix
{ entries, ctxTrace, scopeParent, scopeContexts, scopeEntityKind,
  scopedPipeEffects, scopedClassImports, pipeProducers, pipeConsumers }
```

`class` defaults to `"nixos"`. Use this for fleet-wide pipe views (who
produces what, who consumes it) rather than per-host captures.
